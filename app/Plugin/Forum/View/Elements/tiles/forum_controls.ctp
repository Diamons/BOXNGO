<?php if ($user) {
	...
	<div class="controls <?php echo isset($class) ? $class : ''; ?>">
		...
		if ($isMod) {
			echo $this->Html->link(__d('forum', 'Moderate'), array('controller' => 'stations', 'action' => 'moderate', $forum['Forum']['slug']), array('class' => 'btn btn-small btn-danger'));
		}

		if ($forum['Forum']['status']) {
			if ($this->Forum->hasAccess('Forum.Topic', 'create', $forum['Forum']['accessPost']) || $isMod) {
				echo $this->Html->link(__d('forum', 'Create Topic'), array('controller' => 'topics', 'action' => 'add', $forum['Forum']['slug']), array('class' => 'btn btn-small btn-primary'));
			}

			if ($this->Forum->hasAccess('Forum.Poll', 'create', $forum['Forum']['accessPoll']) || $isMod) {
				echo $this->Html->link(__d('forum', 'Create Poll'), array('controller' => 'topics', 'action' => 'add', $forum['Forum']['slug'], 'poll'), array('class' => 'btn btn-small btn-primary'));
			}
		} else {
			echo '<span class="btn btn-small btn-default">' . __d('forum', 'Closed') . '</span>';
		} ?>
	</div>
<?php } ?>